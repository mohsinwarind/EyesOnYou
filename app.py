...
class FocusVideoProcessor(VideoProcessorBase):
    ...
    def recv(self, frame: av.VideoFrame) -> av.VideoFrame:
        ...
        if self.tracker is not None:
            result = self.tracker.process_frame(img)
            with self._lock:
                self.result = result
...
[data-testid="stSlider"] > div > div > div { background: #d0d0d0 !important; height: 6px !important; }
[data-testid="stSlider"] > div > div > div > div { background: #1a1a1a !important; }
[data-testid="stSlider"] label { font-size: 0.95rem !important; color: #1a1a1a !important; }
...
[data-testid="stFileUploader"] label { font-size: 0.95rem !important; color: #1a1a1a !important; }

label, .stMarkdown p { color: #1a1a1a !important; font-size: 0.95rem !important; font-weight: 500 !important; }
...
[data-testid="stMetricValue"] { color: #1a1a1a !important; font-size: 1.6rem !important; font-family: 'DM Mono', monospace !important; font-weight: 500 !important; }
[data-testid="stMetricLabel"] { color: #666 !important; font-size: 0.8rem !important; letter-spacing: 0.06em; text-transform: uppercase; font-weight: 600 !important; }

[data-testid="stProgressBar"] > div { background: #e0e0e0 !important; border-radius: 6px !important; box-shadow: inset 0 1px 3px rgba(0,0,0,0.05); height: 10px !important; }
[data-testid="stProgressBar"] > div > div { background: #1a1a1a !important; border-radius: 6px !important; }
...
.clock-face {
    ...
    border-radius: 50%;
    box-shadow: 0 0 0 1px #e0e0e0, 0 8px 32px rgba(0,0,0,0.1), inset 0 2px 4px rgba(255,255,255,0.8), inset 0 -2px 8px rgba(0,0,0,0.05);
    display: flex;
    align-items: center;
...
.clock-time { font-family: 'DM Mono', monospace; font-size: 4rem; font-weight: 500; color: #1a1a1a; letter-spacing: 0.02em; }
.clock-label { font-size: 0.8rem; color: #666; letter-spacing: 0.18em; text-transform: uppercase; margin-top: 8px; font-weight: 600; }
.clock-session { font-size: 0.8rem; color: #888; letter-spacing: 0.12em; text-transform: uppercase; margin-top: 4px; font-family: 'DM Mono', monospace; font-weight: 500; }

.status-pill { display: inline-flex; align-items: center; gap: 8px; padding: 6px 16px; border-radius: 20px; font-size: 0.85rem; font-weight: 600; letter-spacing: 0.08em; text-transform: uppercase; }
...
.webcam-label { font-size: 0.8rem; color: #666; letter-spacing: 0.1em; text-transform: uppercase; text-align: center; margin-bottom: 8px; font-weight: 600; }

.setup-title { font-size: 2.5rem; font-weight: 600; color: #1a1a1a; letter-spacing: -0.03em; margin-bottom: 8px; }
.setup-sub { font-size: 1rem; color: #666; margin-bottom: 2.5rem; font-weight: 400; }
.field-label { font-size: 0.85rem; color: #1a1a1a; letter-spacing: 0.05em; text-transform: uppercase; margin-bottom: 8px; font-weight: 600; }
.caption-text { font-size: 0.85rem; color: #666; margin-top: 4px; }

[data-testid="stNumberInput"] { background: #ffffff !important; border-radius: 8px; }
[data-testid="stNumberInput"] label { font-size: 0.95rem !important; color: #1a1a1a !important; }
...
@media (max-width: 768px) {
    ...
    .clock-label, .clock-session { font-size: 0.65rem; }
}
</style>
...
if st.session_state.page == "setup":
    ...
    with col:
        ...
        uploaded = st.file_uploader("Guardian photo", type=["jpg", "jpeg", "png"], label_visibility="collapsed")
        ...
        grace = st.slider("Grace period", min_value=3, max_value=15, value=8, label_visibility="collapsed")
...
# ══════════════════════════════════════════════════════════════
    ...
    .main .block-container { padding-top:0rem !important; padding-bottom:0rem !important; padding-left:1rem !important; padding-right:1rem !important; max-width:100% !important; }
    ...
    with cam_col:
        ...
        # WebRTC — browser streams webcam to server
        ...
        # Attach tracker once stream starts
        if webrtc_ctx.video_processor and st.session_state.tracker:
            webrtc_ctx.video_processor.set_tracker(st.session_state.tracker)
    ...
    def timer_fragment():
        ...
        # Pull latest frame result from WebRTC processor
        ...
        def trigger_guardian(event_type, direction, phone):
            _now = time.time()
            if _now - st.session_state.last_guardian_time < GUARDIAN_COOLDOWN:
                return
            ...
            st.session_state.guardian_active_until = _now + GUARDIAN_DISPLAY_SECS
            st.session_state.last_guardian_time    = _now

            # Browser-based TTS — plays in user's browser, works on cloud
            st.session_state.tts.speak(msg)
            # Music is disabled on cloud (no server audio device)
        ...
        # ── Distraction logic (WORK phase only) ───────────────
        ...
        if st.session_state.guardian_message:
            msg_ph.markdown(
                ...
                f'<span style="font-size:0.75rem;color:#666;letter-spacing:0.1em;'
